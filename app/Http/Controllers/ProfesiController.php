    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($profesi)
            ...
            ->addColumn('action', function ($row) {
                ...
                // warning
                $btn = '<button onclick="modalAction(\'' . url('/profesi/' . $row->profesi_id . '/edit_ajax') . '\')" class="btn btn-warning btn-sm">Edit</button> ';
                // danger
                $btn .= '<button onclick="modalAction(\'' . url('/profesi/' . $row->profesi_id . '/delete_ajax') . '\')" class="btn btn-danger btn-sm">Hapus</button> ';
                return $btn;
            })
            ->rawColumns(['action'])
            ->make(true);
